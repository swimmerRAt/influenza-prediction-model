...
CSV_PATH = Path.cwd() / "influenza_data.csv"
...
def load_and_prepare(df_input: pd.DataFrame = None) -> Tuple[np.ndarray, np.ndarray, list, list]:   
    ...
    if "week_sin" not in df.columns and "week" in df.columns:
        ...
        print(f"   ✅ week_sin 생성")
    if "week_cos" not in df.columns and "week" in df.columns:
    ...
    # 기본 피처
    base_feat_names = ["ili", "delta_ili", "week_sin", "week_cos"]
    
    # peak_gap, detection_rate, 날씨 피처 추가
    optional_feat_names = [
        "peak_gap",
        "detection_rate",
        "min_temp",
    ...
    # 실제 존재하는 피처들만 선택
    feat_names = base_feat_names.copy()
    for feat in optional_feat_names:
        if feat in df.columns:
            feat_names.append(feat)
    
    print(f"   사용 입력 피처: {feat_names} (총 {len(feat_names)}개)")

...
시계열 데이터 기반의 인플루엔자(ILI) 발생률 예측을 위한 **하이브리드 딥러닝 모델**입니다. **CNN(Convolutional Neural Network)**과 **Transformer**를 결합한 PatchTST 모델을 사용하며, PostgreSQL을 활용한 효율적인 대용량 데이터 관리와 결합했습니다.
...
- **모델**: PatchTST (CNN + Transformer 하이브리드 모델)
  - **CNN**: 멀티스케일 특징 추출 (커널 크기 1, 3, 5, dilation=2)
  - **Transformer**: 패치 간 장기 의존성 학습 (4 레이어, 2 헤드)
...
**PatchTST (Patch Time Series Transformer)**는 시계열 데이터를 패치 단위로 나누어 처리하는 하이브리드 모델입니다. **CNN(Convolutional Neural Network)**과 **Transformer**를 결합하여 단기 패턴과 장기 의존성을 동시에 학습합니다.

### 전체 모델 파이프라인

```
┌─────────────────────────────────────────────────────────────────┐
│ 1️⃣ 데이터 입력 (12주 × 6개 특징)                                     │
│    - ili (의사환자 분율), vaccine_rate, respiratory_index 등        │
└─────────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────────┐
│ 2️⃣ 패치 분할 (Patch Splitting)                                     │
│    - 12주 → 3개 패치 (각 패치 = 4주)                                 │ 
│    - Shape: (B, 3, 4, 6) → (배치, 패치수, 패치길이, 특징수)            │
└─────────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────────┐
│ 3️⃣ 멀티스케일 CNN 임베딩 (MultiScaleCNNPatchEmbed)                   │
│    ┌─────────────────────────────────────────────────────┐      │
│    │ 각 패치(4주)에 대해 4개의 CNN 분기 병렬 적용:                │      │
│    │                                                      │      │
│    │  • Branch 1: Conv1d(k=1)  → 단기 급격 변화 포착          │      │
│    │  • Branch 2: Conv1d(k=3)  → 중기 패턴 포착              │      │
│    │  • Branch 3: Conv1d(k=5)  → 장기 트렌드 포착             │      │
│    │  • Branch 4: Conv1d(k=3, dilation=2) → 확장 패턴       │      │
│    │                                                     │      │
│    │ 4개 분기 출력 → Concat → (B, 3, 128)                   │      │
│    └─────────────────────────────────────────────────────┘      │
└─────────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────────┐
│ 4️⃣ 토큰 컨볼루션 믹서 (TokenConvMixer)                               │
│    - 패치 토큰 간 로컬 연속성 강화                                     │
...
└─────────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────────┐
│ 5️⃣ 위치 인코딩 (Positional Encoding)                            │
│    - 패치의 시간적 위치 정보 추가                                │
│    - Sinusoidal encoding 사용                                    │
└─────────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────────┐
│ 6️⃣ Transformer Encoder (4 layers)                              │
│    ┌─────────────────────────────────────────────────────┐      │
│    │ 각 레이어:                                          │      │
...
└─────────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────────┐
│ 7️⃣ 어텐션 풀링 (Attention Pooling)                              │
│    - 3개 패치 토큰을 가중 평균하여 하나의 벡터로 집약             │
│    - Learnable query attention 사용                             │
│    - Shape: (B, 3, 128) → (B, 128)                              │
└─────────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────────┐
...
- **입력**: 12주 × 6개 특징
- **패치 분할**: 12주를 3개 패치로 분할 (각 패치 = 4주)
- **장점**: 
  - 계산 효율성 향상
  - 로컬 패턴과 글로벌 패턴 동시 학습
  - 장기 의존성 학습 용이

#### 2. 멀티스케일 CNN (Multi-Scale CNN) ⭐ 핵심

**MultiScaleCNNPatchEmbed** 클래스가 각 패치에 대해 4개의 CNN 분기를 병렬로 적용합니다:

```python
# 각 패치 (4주 × 6 특징)에 대해:
Branch 1: Conv1d(kernel_size=1)  → 즉각적인 변화 포착
Branch 2: Conv1d(kernel_size=3)  → 단기 패턴 (3주)
Branch 3: Conv1d(kernel_size=5)  → 중기 패턴 (5주, 패딩으로 확장)
Branch 4: Conv1d(kernel_size=3, dilation=2) → 확장된 수용 영역

# 4개 분기 출력을 결합
Output = Concat([Branch1, Branch2, Branch3, Branch4])  # (128 dim)
```

**왜 멀티스케일 CNN인가?**
- 인플루엔자는 다양한 시간 스케일의 패턴을 가짐
  - 단기: 주간 변동성
  - 중기: 계절성 (4-8주)
  - 장기: 연간 트렌드
- 단일 커널 크기로는 모든 패턴을 포착하기 어려움
- 4개 분기를 병렬로 사용하여 다양한 스케일의 특징을 동시에 추출

#### 3. 토큰 컨볼루션 믹서 (TokenConvMixer)

- **목적**: 패치 토큰 간 로컬 연속성 강화
- **구조**: DepthwiseConv1d + PointwiseConv1d
- **효과**: 인접한 패치 간의 시간적 연속성 학습

#### 4. Transformer Encoder

- **레이어 수**: 4개
- **Attention 헤드**: 2개
- **역할**: 패치 간 장기 의존성 및 복잡한 관계 학습
  - 예: "1월 패치"와 "12월 패치" 간의 계절성 관계

#### 5. 어텐션 풀링 (Attention Pooling)

- **목적**: 3개 패치 토큰을 하나의 벡터로 집약
- **방법**: Learnable query를 사용한 가중 평균
- **장점**: 중요한 패치에 더 높은 가중치 부여
...
PRED_LEN = 3        # 예측 길이 (3주)
PATCH_LEN = 4       # 패치 크기 (각 패치 = 4주)
STRIDE = 1          # 패치 간 간격

# 모델 구조
D_MODEL = 128       # 임베딩 차원 (CNN 4개 분기 합산: 32×4=128)
N_HEADS = 2         # Attention 헤드 수
ENC_LAYERS = 4      # Encoder 레이어 수
FF_DIM = 128        # Feed-forward 차원
DROPOUT = 0.3       # 드롭아웃 비율

# CNN 설정 (MultiScaleCNNPatchEmbed 내부)
CNN_KERNELS = [1, 3, 5]  # 커널 크기
CNN_DILATION = 2         # 확장 컨볼루션 dilation
...
| 단계 | 컴포넌트 | 입력 Shape | 출력 Shape | 설명 |
|------|---------|-----------|-----------|------|
| 1 | 데이터 입력 | - | (B, 12, 6) | 12주 × 6개 특징 |
| 2 | 패치 분할 | (B, 12, 6) | (B, 3, 4, 6) | 3개 패치 × 4주 |
| 3 | **멀티스케일 CNN** | (B, 3, 4, 6) | (B, 3, 128) | 4개 CNN 분기 병렬 처리 |
| 4 | 토큰 믹서 | (B, 3, 128) | (B, 3, 128) | 패치 간 연속성 강화 |
| 5 | 위치 인코딩 | (B, 3, 128) | (B, 3, 128) | 시간적 위치 정보 추가 |
| 6 | Transformer Encoder | (B, 3, 128) | (B, 3, 128) | 4개 레이어, 2개 헤드 |
| 7 | 어텐션 풀링 | (B, 3, 128) | (B, 128) | 3개 패치 → 1개 벡터 |
| 8 | MLP 헤드 | (B, 128) | (B, 3) | 3주 예측 |

### CNN vs Transformer 역할 분담

| 컴포넌트 | 역할 | 학습하는 패턴 |
|---------|------|--------------|
| **CNN** | 로컬 패턴 추출 | 패치 내부의 단기/중기/장기 패턴 (1-5주) |
| **Transformer** | 글로벌 의존성 | 패치 간 장기 의존성 및 계절성 (12주 이상) |
...
- **Regularization**: Correlation Loss (예측-실제값 상관관계 유지)
  ```python
  corr_loss = 1 - correlation(predicted, actual)
  total_loss = mae_loss + λ * corr_loss
  ```

### 모델 실행 흐름 예시

```
입력: [Week 1-12의 ILI, 백신률, 호흡기지수 등]
  ↓
패치 1: [Week 1-4]   → CNN → 임베딩 벡터 1
패치 2: [Week 5-8]   → CNN → 임베딩 벡터 2
패치 3: [Week 9-12]  → CNN → 임베딩 벡터 3
  ↓
Transformer가 3개 패치 벡터 간 관계 학습
  ↓
어텐션 풀링으로 최종 표현 벡터 생성
  ↓
출력: [Week 13, 14, 15의 예측 ILI 값]
```

...
- **주요 컬럼**:
  - `year`, `week`: 시간 정보
  - `ili`: 인플루엔자 유사질환 발생률 (타겟 변수)
  - `vaccine_rate`: 백신 접종률
  - `respiratory_index`: 호흡기 질환 지수
  - `temperature`, `humidity`, `rainfall`: 기후 데이터
  - `dataset_id`: 데이터셋 출처
